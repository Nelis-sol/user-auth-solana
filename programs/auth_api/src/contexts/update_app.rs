...
pub struct UpdateApp<'info> {
    ...
    #[account(mut, seeds = [b"app".as_ref(), hash.as_ref()], constraint = identity.uid == app.uid, bump = app.bump)]
    pub app: Account<'info, App>,
    #[account(address = system_program::ID)]
    pub system_program: Program<'info, System>,
}

impl<'info> UpdateApp<'_> {
    pub fn process(&mut self, action: bool, tos: Option<u8>) -> Result<()> {
        ...
        match tos {
            None => (),
            Some(accepted_tos) => {app.accepted_tos = accepted_tos;},
        }
