...
pub struct UpdateProfile<'info> {
    ...
    #[account(mut, seeds = [b"profile".as_ref(), hash.as_ref()], constraint = identity.uid == profile.uid, bump = profile.bump)]
...
impl<'info> UpdateProfile<'_> {
    pub fn process(&mut self, action: bool, tos: Option<u8>) -> Result<()> {
        ...
        match tos {
            None => (),
            Some(accepted_tos) => {profile.accepted_tos = accepted_tos;},
        }
